...
@section('content')
    ...
    <div class="">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('pendaftaran.periode.update', $pendaftaran->id) }}" method="PUT">
                    @csrf

                    <div class="mb-3">
                        <label for="name" class="form-label">Nama</label>
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            id="name" placeholder="John Doe" value="{{ old('name', $pendaftaran->name) }}" />
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="periode" class="form-control @error('periode') is-invalid @enderror"
                            id="periode" placeholder="Select Year" value="{{ old('periode', $pendaftaran->periode) }}" />
                        @error('periode')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
