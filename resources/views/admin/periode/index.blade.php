...
@section('content')
    ...
    <div class="card h-100">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <button onclick="window.location.href='{{ route('pendaftaran.periode.create') }}'" type="button"
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>
                                <!-- Show button -->
                                <button onclick="window.location.href='{{ route('pendaftaran.periode.show', $item->id) }}'"
                                    type="button" class="btn btn-icon btn-outline-info" title="Show">
                                    <span class="tf-icons bx bx-show"></span>
                                </button>
                                <!-- Edit button -->
                                <button onclick="window.location.href='{{ route('pendaftaran.periode.edit', $item->id) }}'"
                                    type="button" class="btn btn-icon btn-outline-warning" title="Edit">
                                    <span class="tf-icons bx bx-edit"></span>
                                </button>
                                <!-- Delete button -->
                                <form action="{{ route('pendaftaran.periode.destroy', $item->id) }}" method="POST"
