            ...
            <div class="card-body">
                <form action="{{ route('alumni.update', $alumni->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        @enderror
                        @if ($alumni->foto)
                            <img src="{{ asset('storage/' . $alumni->foto) }}" alt="Foto" width="100" class="mt-2">
                        @endif
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            id="name" placeholder="John Doe" value="{{ old('name', $alumni->name) }}" />
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="tahun_end" class="form-control @error('tahun_end') is-invalid @enderror"
                            id="tahun_end" placeholder="Select Year" value="{{ old('tahun_end', $alumni->tahun_end) }}" />
                        @error('tahun_end')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('alamat') is-invalid @enderror" name="alamat" id="alamat" rows="3">{{ old('alamat', $alumni->alamat) }}</textarea>
                        @error('alamat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
