...
@section('content')
    ...
    <div class="card h-100">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <button onclick="window.location.href='{{ route('kriteria.sub.create') }}'" type="button"
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->isCF == 1 ? 'True' : 'False' }}</td>
                            <td>
                                <!-- Show button -->
                                <!-- Edit button -->
                                <button onclick="window.location.href='{{ route('kriteria.sub.edit', $item->id) }}'"
                                    type="button" class="btn btn-icon btn-outline-warning" title="Edit">
                                    <span class="tf-icons bx bx-edit"></span>
                                </button>
                                <!-- Delete button -->
                                <form action="{{ route('kriteria.sub.destroy', $item->id) }}" method="POST"
