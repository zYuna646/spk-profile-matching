...
@section('content')
    ...
    <div class="">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('kriteria.sub.update', $sub->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            id="name" placeholder="John Doe" value="{{ old('name', $sub->name) }}" />
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="bobot" class="form-control @error('bobot') is-invalid @enderror"
                            id="bobot" placeholder="" value="{{ old('bobot', $sub->bobot) }}" step="0.01" />
                        @error('bobot')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" name="isCF" class="form-check-input @error('isCF') is-invalid @enderror"
                            id="isCF" value="1" {{ old('isCF', $sub->isCF) ? 'checked' : '' }} />
                    ...
                    <div class="mb-3">
                        ...
                        <select name="kriteria_id" class="form-select @error('kriteria_id') is-invalid @enderror"
                            ...
                            @foreach ($kriterias as $kriteriaItem)
                                <option value="{{ $kriteriaItem->id }}"
                                    {{ old('kriteria_id', $sub->kriteria_id) == $kriteriaItem->id ? 'selected' : '' }}>
                                    {{ $kriteriaItem->name }}</option>
                            @endforeach
                        </select>
                        @error('kriteria_id')
                            <div class="invalid-feedback">{{ $message }}</div>
